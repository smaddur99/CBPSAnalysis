...
create_balance_table_grouped <- function(
  ...
  if (is.data.frame(balance_results)) {
    ...
    if ("balance" %in% names(balance_results) &&
      ...
      # Multiple models - PRESERVE ORDER
      balance_list <- list()
      model_order_map <- setNames(seq_along(names(balance_results)), names(balance_results))
  ...
  # Sort by model_order to preserve list order, then by covariate
  table_data <- balance_data %>%
    dplyr::filter(!is.na(.data$variance_balance) | !is.na(.data$mean_balance)) %>%
    dplyr::arrange(.data$model_order, .data$covariate_name) %>%
  ...
  n_covariates <- length(unique(gsub("\\*", "", table_data$covariate_name)))
